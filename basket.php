...
<h3 id="basket_label" class="text-center mb-5 mt-5"><b>Ваша корзина:</b></h3>
<div class="container">
	<div class="row">
		<?php 
		    ...
		    $_SESSION['baskets']['sums'] = '0'; 
		    $_SESSION['baskets']['amounts'] = '0'; 
		...
		<div class="row">
			<div class="col-sm-4 text-center mb-3 mt-3">
				<img src="db_books_img/<?echo $row['image']?>" class="poster-wh img-fluid rounded-3">
			</div>
			<div class="col-sm-8">
				<table>
					<tr>
						<td></td>
						<td><b>Название книги:</b> <?echo $row['name']?></td>
					</tr>
					<tr>
						<td></td>
					...
					<tr>
						<td></td>
						<td><div class="row">
								<div class="col-sm-4"><b>Количество: </b></div>
								<form action="basket_del_more.php" method="get" class="col-sm-2 text-end my-auto">
									<a href="basket_del_more.php?id=<?echo $row['id']?>" class="navstyles">-</a>
								</form>
								<div class="col-sm-4 text-center"><?echo $row['amount']?> шт.</div>
								<form action="basket_add_more.php" method="get" class="col-sm-2 text-start">
									<a href="basket_add_more.php?id=<?echo $row['id']?>" class="navstyles">+</a>
								</form>
							</div>
						</td>
					</tr>
				...
				<a class="banners_btns p-2" style="text-decoration:none;color: black; font-size: 14px;" href="del_basket.php?id=<?echo $row['id']?>">Удалить!</a>
				<? 
					
					$_SESSION['baskets']['sums'] += $row['price'];
					$_SESSION['baskets']['amounts'] += $row['amount'];
		...
		<?
			...
			<div class="text-end mt-3 mb-3">
				<h5><b>К оплате:</b> <?echo $_SESSION['baskets']['sums'];
				#echo $row['price'] * $row['amount'];
				?> рублей.</h5>
			</div>
			<div class="text-end">
				<button class="banners_btns p-3">Купить!</button>
			</div>
			<?
			}
			 else {?>
				<script>
					var basket_label = document.getElementById("basket_label");
					basket_label.innerHTML = "<b>Ваша корзина пуста!</b>";
				</script>
				<?
			}
			$conn->close();    
